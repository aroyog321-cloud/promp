  ...
  return (
    <AnimatePresence mode="wait">
      <motion.div
        ...
        className={`mt-4 flex items-center justify-between gap-2 px-4 py-3 rounded-xl text-sm font-medium ${
          status === 'synced' ? 'bg-promptly-cyan/10 text-promptly-cyan border border-promptly-cyan/20' : 
          status === 'failed' ? 'bg-red-500/10 text-red-400 border border-red-500/20' : 
          'bg-promptly-violet/10 text-promptly-violet border border-promptly-violet/20'
        }`}
      >
        <div className="flex items-center gap-3">
          {status === 'synced' ? (
            <>
              ...
              >
                <svg className="w-5 h-5 text-promptly-cyan" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <>
              <svg className="w-5 h-5 animate-spin text-promptly-violet" fill="none" viewBox="0 0 24 24">
